...
const LANGUAGES = {
  indian: [
    "Hindi", "Tamil", "Bengali", "Punjabi", "Marathi", "Telugu", "Gujarati", "Malayalam", "Kannada", "English"
  ],
  foreign: [
    "Arabic", "French", "Spanish", "German", "Chinese", "Japanese", "Russian", "Portuguese", "Turkish", "Indonesian"
  ]
};
      ...
      <div className="max-w-5xl mx-auto space-y-16">
        ...
        <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            Global Language <span className="text-blue-500">Scam Detector</span>
          </h1>
          <p className="text-slate-400 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
            Paste a message in any Indian or foreign language. AI will translate it into English and check if it’s a scam.
          </p>
        </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                  <div className="relative w-full sm:w-auto">
                    <select 
                      ...
                      className="w-full sm:w-64 appearance-none bg-slate-800 border border-slate-700 text-slate-200 px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all font-bold cursor-pointer"
                    >
                      <option>Auto Detect</option>
                      <optgroup label="Indian Languages">
                        {LANGUAGES.indian.map(l => <option key={l}>{l}</option>)}
                      </optgroup>
                      <optgroup label="Foreign Languages">
                        {LANGUAGES.foreign.map(l => <option key={l}>{l}</option>)}
                      </optgroup>
                    </select>
            ...
            <div className="grid md:grid-cols-2 gap-16 pt-12 items-center">
              <div>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h2 className="text-3xl font-black text-white">Why Global Detection Matters</h2>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap justify-center gap-3">
                      {LANGUAGES.indian.map(lang => (
                        <span key={lang} className="px-3 py-1 bg-slate-950 border border-slate-800 rounded-lg text-xs font-bold text-slate-400">{lang}</span>
                      ))}
                    </div>
                  </div>
                  <div>
